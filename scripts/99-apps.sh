...
if command -v firefox &>/dev/null; then
    ...
    # 只需要检查 Snippet 是否存在，不需要先运行 Firefox
    if [ -f "$FF_SNIPPET" ]; then
        section "Config" "Firefox UI Customization (Pre-seeding)"
        
        MOZILLA_DIR="$HOME_DIR/.mozilla"
        FF_DIR="$MOZILLA_DIR/firefox"
        # 我们手动指定一个固定的 Profile 名字，比如 "default"
        PROFILE_NAME="default" 
        PROFILE_PATH="$FF_DIR/$PROFILE_NAME"
        
        log "Pre-creating Firefox Profile structure..."
        ...
        # 2. 关键步骤：创建/覆盖 profiles.ini
        # 这告诉 Firefox："去哪里找配置"
        # 即使 Firefox 从未运行过，有了这个文件，它启动时就会直接加载 $PROFILE_PATH
...
EOF
        ...
        # 3. 直接写入 user.js
        # 注意：这里我们不需要备份，因为假设是全新安装。
        # 如果你想兼顾"已存在配置"的情况，可以在这里加个判断。
...
EOF
        ...
        # 5. 统一修正权限 (最重要的一步)
        # 因为上面的 mkdir 和 cat 可能是以 root (脚本用户) 执行的
        # 必须把整个 .mozilla 目录还给目标用户
        log "Setting permissions for $TARGET_USER..."
        chown -R "$TARGET_USER" "$MOZILLA_DIR"
        
        success "Firefox profile configured (Static Generation)."
